    ...
    <?php if(session('cart') && count(session('cart')) > 0): ?>
        <div class="checkout-form">
            ...
            <ul class="list-group">
                ...
                <?php $__currentLoopData = session('cart'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $id => $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li class="list-group-item d-flex justify-content-between">
                        <span><?php echo e($product['name']); ?> (x<?php echo e($product['quantity']); ?>)</span>
                        <span>₱<?php echo e(number_format($product['price'] * $product['quantity'], 2)); ?></span>
                    </li>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </ul>

            <p class="mt-3"><strong>Total:</strong> ₱<?php echo e(number_format($total, 2)); ?></p>
            ...
            <form id="checkout-form" action="<?php echo e(route('payment.process')); ?>" method="POST">
                ...
                <input type="hidden" name="cart" value="<?php echo e(json_encode(session('cart'))); ?>" />

                <div class="form-group">
                    <label for="full_name">Full Name</label>
                    <input type="text" id="full_name" name="full_name" class="form-control"
                           value="<?php echo e(old('full_name') ?? (auth()->user() ? auth()->user()->name : '')); ?>" required>
                </div>

                <div class="form-group">
                    <label for="email">Email Address</label>
                    <input type="email" id="email" name="email" class="form-control"
                           value="<?php echo e(old('email') ?? (auth()->user() ? auth()->user()->email : '')); ?>" required>
                </div>
        ...
        paypal.Buttons({
            ...
            onApprove: function (data, actions) {
                return actions.order.capture().then(function (details) {
                    ...
                    fetch("<?php echo e(route('payment.process')); ?>", {
                        method: "POST",
                        body: formData,
...
<?php /**PATH C:\xampp\htdocs\LABWORKSY\resources\views/checkout/index.blade.php ENDPATH**/ ?>